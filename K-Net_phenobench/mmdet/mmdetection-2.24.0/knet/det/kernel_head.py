...
class ConvKernelHead(nn.Module):
    ...
    def forward_train(self,
        ...
        # print(scaled_mask_preds.size(),cls_scores.size(),scaled_seg_preds.size(),"     lossesv")
        losses = self.loss(scaled_mask_preds, cls_scores, scaled_seg_preds,
                           proposal_feats, *mask_targets)  # this
        # aaa
    ...
    def loss(self,
        ...
        if seg_preds is not None:
            if self.loss_seg.use_sigmoid:
                ...
                # print(flatten_seg.size(),flatten_seg_target.size(),"     lossesv")
                # aaa
                losses['loss_rpn_seg'] = self.loss_seg(  # this
                    flatten_seg,
                    flatten_seg_target,
