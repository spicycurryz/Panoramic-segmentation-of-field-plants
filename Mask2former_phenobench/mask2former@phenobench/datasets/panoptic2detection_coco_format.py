...
@get_traceback
def convert_panoptic_to_detection_coco_format_single_core(proc_id, annotations_set, categories, segmentations_folder, things_only):
...
def convert_panoptic_to_detection_coco_format(input_json_file, segmentations_folder, output_json_file, categories_json_file, things_only):
    ...
    for proc_id, annotations_set in enumerate(annotations_split):
        p = workers.apply_async(
            convert_panoptic_to_detection_coco_format_single_core, (proc_id, annotations_set, categories, segmentations_folder, things_only)
        )
        processes.append(p)
...
if __name__ == "__main__":
    ...
    root = "/root/autodl-fs/PhenoBench"
    input_json_file = os.path.join(root, "annotations", "plants_panoptic_train.json")
    output_json_file = os.path.join(root, "annotations", "plants_instance_train.json")
    categories_json_file = "datasets/categories.json"
    segmentations_folder = os.path.join(root, "plants_panoptic_train")

    convert_panoptic_to_detection_coco_format(input_json_file, segmentations_folder, output_json_file, categories_json_file, args.things_only)
